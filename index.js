...
// API Health check specifically for serverless
...
app.get("/api/check-session/:session_id", async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res.status(404).json({
          ...
          message:
            "The payment session could not be found. It may have expired.",
          session_id: session_id,
        });
      }
...
app.get("/api/user/:email/subscription-status", async (req, res) => {
  try {
...
app.get("/api/user/:email/payments", async (req, res) => {
  try {
    const { email } = req.params;
...
app.use((req, res) => {
  res.status(404).json({
    ...
    available_endpoints: [
      ...
      "GET /api/check-session/:session_id",
      "POST /api/verify-session",
      "GET /api/admin/payments",
      "GET /api/admin/active-sessions",
      "POST /api/admin/clear-session",
      "GET /api/user/:email/subscription-status",
      "GET /api/user/:email/payments",
      "GET /api/admin/export-payments",
    ],
  });
});

// For serverless environments (like Vercel), we need to handle this differently
